...
public class CertificatoImpl implements ICertificatoServices{
	...
	@Override
	public int create(CertificatoRequest req) throws AcademyException {
		...
		return certificatoRepository.save(cer).getId();
		
	}
	
	@Override
	public void update(CertificatoRequest req) throws AcademyException {
		
		log.debug("Update: " + req);
		
		Optional<Certificato> c = certificatoRepository.findById(req.getId());
		
		if(c.isEmpty()) throw new AcademyException("Certificato non trovato nel database: " + req.getId());
		
		Certificato certificato = c.get();
		...
		certificatoRepository.save(certificato); // Eseguo l'update
	...
	public void delete(CertificatoRequest req) throws AcademyException {
		...
		if (c.isEmpty()) throw new AcademyException("Certificato non trovato :" + req.getId());
